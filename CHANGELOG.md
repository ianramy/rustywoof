...
This changelog documents the main releases of Rustywoof.
...
## Release v0.3.0 - Test-Driven & Modularized Update - 01/09/2026

Rustywoof `v0.3.0` is a test-driven and modularized update that improves lockfile parsing, adds structured deserialization, and refactors the codebase into a more maintainable structure. Tests were added to ensure the stability and correctness of the updated code to achieve feature functionality. The modularized structure allows for easier maintenance and future feature additions since folders are organized by feature-base.

### Developer Experience
* **Test-Driven Development:** The codebase now uses test-driven development (TDD) to ensure the stability and correctness of the updated code.
* **User Interface:** The CLI now provides a structured, human-readable output for audit results, including package names, versions, and any detected vulnerabilities and published date of the vulnerability together with a clickable link to the vulnerability's OSV page.
* **Python Compatibility:** The codebase now is accessible to Python users via the `rustywoof` package on PyPI.

### Bug Fixes
* **Update Feature:** Fixed a bug in the update feature that was using `self-update` crate to update the CLI itself.
* **Peer Dependency Parsing Bug:** Fixed a bug in the `woof sniff` feature that caused incorrect peer dependency parsing.
